...
export const Content: React.FC<{ id: ExpandTask["id"] }> = ({ id }) => {
  const navigate = useNavigate();

  const { pool, completionId } = useSelector(
    (state: RootState) => state.expandTasks
  );
  const { isSuccess, isError } = useSelector(
    (state: RootState) => state.uploadTaskFile
  );
  ...
  function onSubmit() {
    if (value) {
      dispatch(
        uploadTaskFileModel.thunks.uploadTaskFile({
          file: value,
          completionId,
        })
      );
  ...
  return (
    <TransitionFade>
      ...
      {step === Step.RESPONSE && (
        <TaskResponse key={"Response"} award={taskData.reward} />
      )}
    </TransitionFade>
  );
};
