...
export const Validation = () => {
  ...
  const { pool, isPending } = useSelector(
    (state: RootState) => state.validationPool
  );
  const { isSuccess } = useSelector(
    (state: RootState) => state.validationResult
  );
  const dispatch = useDispatch<AppDispatch>();

  // Placeholder for styling override
  ...
  useEffect(() => {
    ...
    show();
    dispatch(
      validationPoolModel.thunks.fetch({
        id: Number(params.id),
      })
    );
    ...
    <PageLayout
      ...
      Content={
        <TransitionFade>
          {!isSuccess && (
            <div key={"Content"}>
              <ValidationHeader id={Number(params.id)} />
              <ValidationData id={Number(params.id)} />
              <ValidationDetails id={Number(params.id)} />
              <ValidationRate id={Number(params.id)} />
              <ValidationSubmit id={Number(params.id)} />
            </div>
          )}
          {isSuccess && (
            <TaskResponse
