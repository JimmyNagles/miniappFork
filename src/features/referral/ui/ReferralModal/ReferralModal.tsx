...
export const ReferralModal: React.FC<ReferralModalProps> = ({
  ...
  const { isPending, data, isNeedFetchViewer } = useSelector(
    (state: RootState) => ({
  ...
  // Placeholder: will replace Telegram logic with custom share functionality
  // TODO: implement shareLink functionality (e.g., via wallet or Web Share API)
  const shareLink = (url: string) => {
    console.log("[shareLink placeholder]", url);
    // e.g., navigator.share({ url }) in future
  };

  // TODO: implement shareToStory functionality (e.g., custom UI or onchain badge share)
  const shareToStory = (storyContent: any, options?: any) => {
    console.log("[shareToStory placeholder]", { storyContent, options });
  };
      ...
      <TransitionFade className={styles.wrapper}>
        {isPending && (
          <SkeletonWrapper key={"Skeleton"}>
            <div className={styles["cell-skeleton"]} />
            <ButtonSkeleton className={styles["modal-button"]} />
            <ButtonSkeleton className={styles["modal-button"]} />
          </SkeletonWrapper>
        )}
        {!isPending && (
          <div ref={ref} key={"Content"}>
            ...
            <Button
              ...
              icon={"link"}
              onClick={() =>
                shareLink(`${BOT_LINK}?startapp=${data.referralCode}`)
              }
            >
              Share Link
            </Button>
