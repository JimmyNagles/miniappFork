...
export const DownloadCard: React.FC<PropsDefault> = ({ className }) => {
  const { isPending, ios, android, apk } = useSelector(
    (state: RootState) => state.download
  );
  const dispatch = useDispatch<AppDispatch>();

  // TODO: replace openLink with wallet-aware or OS-specific handler
  ...
  return (
    <Card className={className} view={"dark"} size={"s"}>
      <h3 className={styles.title}>Download DataHub</h3>
      <p className={styles.description}>
        Download the DataHub app and enjoy the full experience of DataHub along
        with big and exciting rewards!
      </p>
